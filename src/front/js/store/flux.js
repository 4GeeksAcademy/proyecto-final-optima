const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			deleteMovement: async (movementId, params) => {
				const myHeaders = new Headers();
				...
				try {
					...
					if(response.status === 200){
						...
						// 	getActions().debit(parseInt(result.amount), body.accountId)
						// } else if ((balanceType === "ingreso")) {
						// 	getActions().deposit(parseInt(result.amount), body.accountId)
						// }
						(async () => {
							localStorage.removeItem("userAccounts")
						})()
						await getActions().getDetailsUser();
						await getActions().getAccountsDetail(params)
						await getActions().getAccountsUser()
					}
				} catch (error) {
					console.error(error);
